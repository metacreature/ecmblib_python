...
class ecmbNavigationBase():
    ...
    def _set_target(self, target: str|ecmbImage) -> None:
        target_image_obj = None
        if type(target) == ecmbImage or (type(target) == str and target != ''):
            target_image_obj = self._book_obj.int_get_content(target)
        else:
            ecmbUtils.raise_exception('target must be either an unique_id or an ecmbImage on navigation-item "' + self._label + '"!', 1)
